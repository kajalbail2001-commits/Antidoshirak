...
const Proposal: React.FC<ProposalProps> = ({ 
  ...
  const [isSending, setIsSending] = useState(false); // Лоадер для бота
  const [isExporting, setIsExporting] = useState(false); // Лоадер для скачивания
  ...
  const handleDownload = async () => {
    ...
    try {
        ...
        // ФИКС ДЛЯ МОБИЛОК: добавляем в DOM перед кликом
        ...
        alert("Ошибка генерации. Попробуйте кнопку 'В ЛИЧКУ'.");
    } finally {
        setIsExporting(false);
    }
  };

  // --- МЕТОД 2: ОТПРАВИТЬ СЕБЕ (БОТ) ---
  const handleSendToBot = async () => {
    ...
    try {
        const canvas = await html2canvas(proposalRef.current, {
            backgroundColor: '#050505',
            scale: 1, // Оптимизация размера
            useCORS: true,
            logging: false,
            ignoreElements: (el) => el.classList.contains('no-screenshot')
        });
        
        // Сжимаем в JPEG 0.6 чтобы пролезло
        ...
        alert("✅ Смета отправлена в чат с ботом!");
    } catch (error: any) {
  ...
  // --- ПОЛНЫЙ ТЕКСТОВЫЙ ОТЧЕТ (ВЕРНУЛ КАК БЫЛО) ---
    ...
    <div ref={proposalRef} className={`space-y-6 pb-32 animate-fade-in ${isClientMode ? 'pt-0' : ''} bg-cyber-black min-h-screen text-gray-300 print:bg-white print:text-black print:pb-0 print:space-y-4 print:block`}>
      ...
      <div className={`flex items-center justify-between border-b border-cyber-dim pb-4 ${(!creatorName && !clientName) ? '' : 'pt-0'} px-4 sm:px-0 no-screenshot`}>
        ...
        <div className="flex gap-4 items-center">
            {isClientMode && onFork && (
                <button type="button" onClick={onFork} className="text-xs font-mono text-cyber-tech hover:text-white underline uppercase">
                    🛠 СОЗДАТЬ КОПИЮ
                </button>
            )}
            {!isClientMode && onBack && (
            <button type="button" onClick={onBack} className="text-xs font-mono text-cyber-dim hover:text-white underline">РЕДАКТОР</button>
            )}
        </div>
      </div>
      ...
      {/* --- CREATOR ACTIONS (SCREEN) - FIXED LAYOUT --- */}
      {!isClientMode && (
        <div className="grid grid-cols-2 gap-2 no-print px-4 sm:px-0 no-screenshot">
            <div className="col-span-2 flex gap-2">
                ...
                {/* 👇 ВЕРНУЛ ЭТУ КНОПКУ КАК БЫЛО (БОЛЬШАЯ) И ДОБАВИЛ type="button" */}
                {onShareClick && (
                    <button 
                    type="button"
            ...
            {/* 👇 НОВАЯ КНОПКА ДЛЯ NEKOGRAM */}
            <button 
                type="button"
                disabled={isSending}
                onClick={handleSendToBot}
                className="bg-zinc-900 text-gray-300 border border-zinc-700 font-bold py-3 font-mono uppercase text-[10px] hover:text-white hover:border-white transition-all"
            >
                {isSending ? '...' : '📥 СЕБЕ'}
            </button>
