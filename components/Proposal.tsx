...
const Proposal: React.FC<ProposalProps> = ({ 
  ...
  const handleSendToChat = async () => {
    ...
    if (!tgInitData) {
        alert("⚠️ Работает только внутри Telegram (через бота).");
        return;
    }
    ...
    try {
        const canvas = await html2canvas(proposalRef.current, {
            ...
            // 🔥 ВАЖНО: Scale 1 (не 2). Это уменьшает кол-во пикселей в 4 раза.
            // На мобилке все равно будет видно нормально.
            scale: 1, 
            useCORS: true,
            logging: false,
        ...
        // 🔥 ВАЖНО: JPEG с качеством 0.6. Это уменьшает вес файла в 10 раз по сравнению с PNG.
        const imageBase64 = canvas.toDataURL('image/jpeg', 0.6);

        // Лог для проверки размера (видно в консоли браузера)
        const sizeInKB = Math.round(imageBase64.length * 3 / 4 / 1024);
        console.log(`Размер отправляемой картинки: ${sizeInKB} KB`);

        if (sizeInKB > 5500) {
            alert(`Внимание: Картинка слишком большая (${sizeInKB} KB). Попробуйте уменьшить смету.`);
            setIsSending(false);
            return;
        }
        ...
        if (!response.ok) {
            // Читаем ошибку безопасно
            const text = await response.text();
            let errorMessage = "Ошибка сервера";
            try {
                errorMessage = JSON.parse(text).error;
            } catch (e) {
                // Если пришел не JSON, значит сервер упал (500/502) из-за лимитов
                errorMessage = `Сервер сбросил соединение (Код ${response.status}). Вероятно, картинка всё еще тяжелая.`;
            }
...
export default Proposal;    } finally {
        setIsSending(false);
    }
  };
