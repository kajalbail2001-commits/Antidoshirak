...
const Proposal: React.FC<ProposalProps> = ({ 
  ...
  // --- МЕТОД 1: ОТПРАВКА БОТУ (С УМНЫМ СЖАТИЕМ) ---
  const handleSendToBot = async () => {
    if (!proposalRef.current) return;
    const tgInitData = window.Telegram?.WebApp?.initData;
    
    if (!tgInitData) {
        alert("⚠️ Эта кнопка работает только внутри Telegram (через бота).");
        return;
    }
    ...
    try {
        const canvas = await html2canvas(proposalRef.current, {
            ...
            scale: 1, // Базовое качество
            useCORS: true,
            logging: false,
            ignoreElements: (el) => el.classList.contains('no-screenshot')
        });
        
        // АВТО-СЖАТИЕ: Уменьшаем качество, пока файл не станет < 3.5 МБ
        let quality = 0.8;
        let imageBase64 = canvas.toDataURL('image/jpeg', quality);
        
        // Проверка размера (в МБ)
        while (imageBase64.length > 4500000 && quality > 0.1) {
            quality -= 0.1;
        ...
        if (!response.ok) throw new Error("Сервер не принял файл (всё еще большой?)");
        
        alert("✅ Смета отправлена ботом в личку!");
    } catch (error: any) {
      ...
      {!isClientMode && (
        <div className="grid grid-cols-2 gap-2 no-print px-4 sm:px-0 no-screenshot">
            <div className="col-span-2 flex gap-2">
                {/* 1. КНОПКА ОТКРЫТИЯ МОДАЛКИ (PROTOCOL UPLINK) */}
                {onShareClick && (
                    <button 
                    type="button" 
...
export default Proposal;            let errorMessage = "Ошибка сервера";
            try {
