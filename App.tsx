...
import { AppSettings, PriceData, ProjectItem, RiskLevel, UrgencyLevel, SharedQuote, Tool } from './types';
...
function App() {
  ...
  useEffect(() => {
    ...
    items.forEach(item => {
      ...
      if (item.unit === 'second') {
        ...
        // Images/Text/Generations: Treat as 30 seconds of "content/attention" time 
        // to prevent 1 hour of work on 1 image from triggering "High Pace".
        durationSec = item.amount * 30;
      }
  ...
  const handleAddCustomTool = (tool: Tool) => {
    // 1. Add to settings (persistence)
    setSettings(prev => ({
      ...prev,
      customTools: [...(prev.customTools || []), tool]
    }));
    // 2. Add to current items
    handleAddItems([{ ...tool, uniqueId: Math.random().toString(36).substr(2, 9), amount: 1 }]);
  };
  ...
  const handleProcessBrief = async () => {
    ...
    try {
      const allTools = [...data.tools, ...(settings.customTools || [])];
      const parsedItems = await parseBriefWithGemini(brief, allTools, "");

      const newProjectItems: ProjectItem[] = parsedItems.map(p => {
        let toolDef = allTools.find(t => t.id === p.tool_id);

        // Handle AI-created custom items (temporary or auto-save?)
        // Strategy: If AI suggests a custom item (marked by magic ID or flag), 
        // we create a new tool on the fly.
        if (!toolDef && p.tool_id === 'custom' && p.custom_name) {
          const newTool: Tool = {
            id: `custom_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`,
            name: p.custom_name,
            lightning_price: p.custom_price || 0,
            unit: (p.custom_unit as any) || 'generation',
            category: 'other'
          };
          // Side effect: Save this new tool to settings so it persists? 
          // Yes, let's auto-save useful AI discoveries.
          setSettings(prev => ({
            ...prev,
            customTools: [...(prev.customTools || []), newTool]
          }));
          toolDef = newTool;
        }
  ...
  return (
    <div className="min-h-screen bg-cyber-black text-gray-300 pb-20 font-sans">
      ...
      {showWizard && (
        <ToolWizard
          tools={[...data.tools, ...(settings.customTools || [])]}
          onAdd={(item) => { handleAddItems([item]); setShowWizard(false); }}
          onAddCustom={(tool) => { handleAddCustomTool(tool); setShowWizard(false); }}
